...
def _metadata(dist: Path) -> dict[tuple[str, str], dict[str, Any]]:
    ...
    for architecture in ARCHITECTURES:
        ...
        report_tools = {item.get("tool_id") for item in report.get("tools", []) if isinstance(item, dict)}
        ...
        for tool_id in TOOL_IDS:
            ...
            if (
                ...
                or item.get("native_verified") is not True
            ):
...
def prepare_release(dist: Path, tag: str, notes_path: Path) -> dict[str, Any]:
    ...
    index = {
        ...
        "release_tag": tag,
        "verification": {
            "amd64": "native GitHub runner ubuntu-24.04",
            "arm64": "native GitHub runner ubuntu-24.04-arm",
        },
    ...
    lines = [
        ...
        "",
        "Both architectures ran the real shared capability contract on native GitHub-hosted runners; arm64 was not",
        "accepted from extraction-only or QEMU-only evidence.",
        "",
