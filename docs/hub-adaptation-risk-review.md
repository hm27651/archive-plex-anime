...
>
> 状态：已修复并提交；持续验证由 GitHub Actions 执行。本文同时记录修复结果和交付边界。
...
- [x] 关键 Hub 适配测试已提交 Git；
- [x] 公开纯 Python 回归测试不依赖本机配置即可运行；
- [x] 本机 Ruff、完整测试和 `git diff --check` 通过；
- [x] 提交内容不含凭据、缓存、数据库、日志或本机构建产物。
...
- P3：能力、元数据增量前检和轻量工作流三份纯 Python 回归测试已纳入 Git，并由测试自身注入最小配置；继续排除真实媒体样本和本机私有夹具。
