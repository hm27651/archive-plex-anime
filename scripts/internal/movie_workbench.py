...
from archive_rules import artifact_output_root, movie_video_path, movie_video_relative, tv_video_path, tv_video_relative, temporary_path
from internal.batch_workbench import same_track, track_properties, channel_title
...
from internal.subtitle_pipeline import parse_ass_font_requirements, font_file_records, normalize_font_name
...
def _subtitle_labels(name: str) -> tuple[str, str]:
    ...
    suffix = re.search(r'\.(jpsc|jptc|jasc|jatc|sc|tc)(?:\.ass|\.ssa|\.srt)$', name, re.I)
    group = re.match(r'\[([^\]]+)\]', name)
    return ({'jpsc':'JASC','jptc':'JATC'}.get(suffix[1].lower(), suffix[1].upper()), group[1] if group else '') if suffix else ('','')
...
def inspect_sources(work: Path, config: dict, selected_paths: list[str] | None = None) -> dict:
                ...
                if path.suffix.lower() in MEDIA_SUFFIXES:
                    ...
                    for track in raw.get("tracks", []):
                        ...
                        normalized = {
                            ...
                            "dimensions": props.get("pixel_dimensions"),
                            "fonts": [],
                        }
                    ...
                    item['duration_ns'] = raw.get('container', {}).get('properties', {}).get('duration')
                    item['attachments'] = raw.get('attachments', [])
                else:
    ...
    inventory = {
        ...
        'font_files': [p.relative_to(root).as_posix() for p in root.rglob('*') if p.is_file() and not p.is_symlink() and p.suffix.lower() in {'.ttf', '.otf', '.ttc', '.otc'}],
    }
    for item in result:
...
def recommend_audio(inventory: dict, video: str, language: str = "jpn") -> list[dict]:
    ...
    return [{"source": video, "track": t["id"]} for t in sorted(candidates, key=lambda t: (not any(c in t['codec'].upper() for c in ('FLAC','TRUEHD','PCM','DTS-HD')), -(t.get("channels") or 0)))]
...
def build_movie_plan(work: Path, manifest: dict, decisions: dict, completed_steps=None) -> dict:
    ...
    is_tv = bool(decisions.get('batch_workbench')) and manifest.get('route', {}).get('branch') == 'anime'
    ...
    if decisions.get('batch_features', {}).get('subtitles'):
        selected_groups = decisions.get('batch_subtitle_groups') or []
        if not selected_groups:
            issues.append({'code': 'BATCH_SUBTITLE_GROUP_REQUIRED', 'detail': '外挂字幕已开启，请添加字幕组或关闭外挂字幕。'})
        for target in targets:
            for group in selected_groups:
                sources = {f.get('id') or f.get('path') for f in group.get('files', [])}
                matched = any(s.get('batch_group') == group.get('id') or s.get('source') in sources for s in target.get('subtitles', []))
                if not matched and group.get('id') not in (target.get('subtitle_skips') or []):
                    issues.append({'code': 'BATCH_SUBTITLE_MAPPING_REQUIRED', 'target_id': target.get('id'), 'source': target.get('video'), 'detail': '字幕组尚未对应到本视频，请选择字幕或明确跳过。'})
    ...
    selected_fonts = [source_path(work, name) for name in movie.get('font_sources', [])]
    font_aliases = {normalize_font_name(name) for path in selected_fonts for record in font_file_records(path) for name in record.get('names', [])}
    manifest['discovery'].setdefault('movieSourceFiles', []).extend(file_signature(path) for path in selected_fonts)
    ...
    for target in targets:
        ...
        part = str(target.get('episode') or '').upper() if is_tv else str(target.get("part") or "").lower()
        ...
        if (
            ...
            or (is_tv and not re.fullmatch(r'S\d{2}E\d{2,3}', part))
            or (not is_tv and part and not re.fullmatch(r"cd[1-9]\d*", part))
        ...
        if selected_audio and sum(same_track(ref, default_audio) for ref, _, _ in selected_audio) != 1:
        ...
        video_track = track_properties(videos[0], target.get('video_properties') or {}, name=release or (videos[0]['title'] if decisions.get('batch_workbench') else ''), default=True)
        ...
        arguments += [
            ...
            "--track-name",
            f"{vi}:{video_track['title']}",
            "--default-track-flag",
            f"{vi}:{'yes' if video_track['default'] else 'no'}",
            "--forced-display-flag",
            f"{vi}:{'yes' if video_track['forced'] else 'no'}",
        ]
        ...
        expected.append(
            {
                ...
                "name": video_track['title'],
                "default": video_track['default'],
                "forced": video_track['forced'],
            }
        )
        input_index = 1
        for ref, f, t in selected_audio:
            tid = str(t["mux_id"])
            default = same_track(ref, default_audio)
            t = track_properties(t, ref, name=channel_title(t) if decisions.get('batch_workbench') and not is_tv else t['title'], default=default)
            arguments += [
                "--no-video",
                "--audio-tracks",
                tid,
        ...
        duplicates = {}
        attachment_sources, attachment_names = set(), set()
        for sub_index, ref in enumerate(target.get("subtitles", [])):
            ...
            preserve = bool(ref.get('preserve'))
            if (not label or not group) and not preserve:
                issue("MOVIE_SUBTITLE_INFO_REQUIRED", target, source=f["id"])
                continue
            group = _safe_label(group or '内封字幕', "字幕组")
            label = _safe_label(label or 'SUB', "字幕类型")
            ...
            name = str(ref.get('title', t.get('title', '') if preserve and t else name))
            ...
            if is_ass and not preserve:
                ...
                if is_tv:
                    sub_name = f'S{int(part[1:3])}/{label} {group}{suffix}/{title}.{part}.ass'
            ...
            keep_attachments = preserve and embedded and f['id'] not in attachment_sources
            if keep_attachments:
                attachment_sources.add(f['id'])
                attachment_names.update(a['file_name'] for a in f.get('attachments', []) if a.get('file_name'))
            if preserve and not embedded and is_ass and f.get('fonts') and not movie.get('font_sources'):
                issue('FONT_NOT_FOUND', target, source=f['id'], detail='保留已有字幕时，请选择配套字体，或改用生成子集化字幕。')
            elif preserve and not embedded and is_ass:
                for font in f.get('fonts', []):
                    if font['normalized'] not in font_aliases:
                        issue('FONT_NOT_FOUND', target, source=f['id'], font=font['name'], detail='已选字体不能覆盖这份字幕。')
            lang = str(ref.get("language") or (t["language"] if t else "chi"))
            arguments += [
                "--no-video",
                "--no-audio",
                "--no-chapters",
                *([] if (is_ass and not preserve) or keep_attachments else ["--no-attachments"]),
                "--subtitle-tracks",
                tid,
                "--language",
                f"{tid}:{lang}",
                ...
                f"{tid}:{name}",
                "--default-track-flag",
                f"{tid}:{'yes' if ref.get('default', sub_index == 0) else 'no'}",
                "--forced-display-flag",
                f"{tid}:{'yes' if ref.get('forced', False) else 'no'}",
                str(sub_path),
            ]
            order.append(f"{input_index}:{tid}")
            input_index += 1
            expected.append(
                {
                    ...
                    "language": lang,
                    "name": name,
                    "default": bool(ref.get('default', sub_index == 0)),
                    "forced": bool(ref.get('forced', False)),
        ...
        for font in movie.get('font_sources', []):
            font_path = source_path(work, font)
            if font_path.suffix.lower() not in {'.ttf', '.otf', '.ttc', '.otc'}:
                issue('FONT_NOT_FOUND', target, source=font)
                continue
            arguments += ['--attach-file', str(font_path)]
            attachment_names.add(font_path.name)
        arguments += ["--track-order", ",".join(order)]
        chapters = bool(video_file.get("chapters")) and decisions.get("keep_chapters", True)
        season, episode = (int(part[1:3]), int(part[4:])) if is_tv else (0, 0)
        output = tv_video_path(output_root, title, season, episode) if is_tv else movie_video_path(output_root, title, part)
        job = {
            "targetId": target_id,
            ...
            "expectedAttachments": sorted(attachment_names),
        ...
        finals.append(
            {
                "source": str(output),
                "relativePath": tv_video_relative(title, season, episode) if is_tv else movie_video_relative(title, part),
                "expectedTracks": expected,
                "expectedChapters": chapters,
                "expectedAttachments": sorted(attachment_names),
                "allowSelectedCommentary": True,
    ...
    archive_root = config.get("paths", {}).get('subtitleArchiveRoot' if is_tv else "movieSubtitleArchiveRoot")
    final_zip = []
    ...
    plan = {
        "title": title,
        "preferredLibrary": (decisions.get("library_target") or {}).get("library") or ('Anime1' if is_tv else "Movie1"),
